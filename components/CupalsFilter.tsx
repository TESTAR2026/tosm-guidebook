...
import { GRADES, ATTRIBUTES, GRADE_STYLES, CUPAL_RACES } from "@/lib/constants";
...
export function CupalsFilter({ search, setSearch, grade, setGrade, attribute, setAttribute, race, setRace }: CupalsFilterProps) {
        ...
        {CUPAL_RACES.map((r) => (
          <button
            ...
            className={`h-7 px-3 rounded-full text-xs font-semibold transition-all ${
              race === r
                ? "bg-indigo-100 text-indigo-700 border border-indigo-200 ring-2 ring-inset ring-amber-400"
                : "bg-gray-100 text-gray-600 hover:bg-gray-200"
            }`}
          >
            {r}
